...
# ==============================================================================
# Combines disease diagnoses, socioeconomic panel data, and comorbidity indices
# to create analysis-ready datasets for income loss study.
#
# Inputs: DBCG cancer registry, LPR conditions, panel data, comorbidity scores
# Output: Analysis dataset with standardized variables and factor coding
# ==============================================================================
...
# ------------------------------------------------------------------------------
# LOAD SUPPLEMENTAL DATA FOR ECONOMIC ADJUSTMENTS
# ------------------------------------------------------------------------------
# Load Consumer Price Index (CPI) and EUR/DKK exchange rates for inflation adjustment
...
# ------------------------------------------------------------------------------
# PROCESS DISEASE/CONDITION DATA
# ------------------------------------------------------------------------------
# Load breast cancer diagnoses from DBCG registry
dbcg <- open_dataset("data/rawdata/dbcg/") %>%
  select(pnr = CPR, dbcg_dx_yr = Aar) %>%
  collect() %>%
  setDT()

# Process LPR-defined conditions (depression, stroke, alcohol disorders)
conditions <- c("ddd","dap","nab")  # Depression, stroke, alcohol

# Function to process condition data with 5-year separation rule
read_lpr_conditions <- function(cond){
  dt <- open_dataset(paste0("data/intermediates/populations/",cond,"/")) %>% 
    select(pnr, year) %>% collect() %>% setDT()
  dt <- unique(dt)
  
  # Apply 5-year separation rule: only keep recurrent episodes ≥5 years apart
  # This prevents double-counting of chronic conditions or short-term relapses
...
# Extract each condition with 5-year separation applied
ddd <- read_lpr_conditions("ddd")  # Depression diagnoses
dap <- read_lpr_conditions("dap")  # Stroke diagnoses  
nab <- read_lpr_conditions("nab")  # Alcohol disorder diagnoses

# Combine all disease/condition diagnoses into master lookup table
all_dx <- merge(dbcg, ddd, by = "pnr", all = TRUE) %>%
  merge(dap, by = "pnr", all = TRUE) %>%
  merge(nab, by = "pnr", all = TRUE)

# ------------------------------------------------------------------------------
# PROCESS PANEL DATA BY YEAR (2000-2023)
# ------------------------------------------------------------------------------
# Load and process socioeconomic panel data year by year
years <- 2000:2023
...
# Load panel data for current year
df <- open_dataset(paste0(panel_data_dir, "99_panel",yr,".parquet"))

# Define core variables needed for analysis (demographics, socioeconomic, health)
...
# Filter to Danish residents aged 18+ and select core variables
df <- df %>% 
  filter(in_dk == 1 & de_age >= 18) %>%
  select(all_of(intersect(all_columns, names(df)))) %>%
  collect() %>% setDT()

# Handle missing columns (add as zeros) and standardize column order
...
# Standardize variable names for analysis consistency
setnames(df, 
         all_columns,
...
# ------------------------------------------------------------------------------
# MERGE COMORBIDITY AND DIAGNOSIS DATA  
# ------------------------------------------------------------------------------
# Load comorbidity scores from 2 years after current year (December)
comorbidities <- open_dataset("data/intermediates/comorb/") %>% 
  filter(year == yr + 2 & month == 12) %>%
  collect() %>% setDT()
comorbidities[, year := NULL][, month := NULL]

# Merge comorbidity scores and disease diagnoses with panel data
...
# ------------------------------------------------------------------------------
# VARIABLE RECODING AND FACTOR CREATION
# ------------------------------------------------------------------------------
...
# Recode employment status categories (collapse subcategories of employed)
...
# Create categorical healthcare utilization variables (0, 1, 2, 3, 4+ visits)
df[, util := util_psych + util_somat]  # Total utilization
...
# ------------------------------------------------------------------------------
# Convert all monetary values to 2023 EUR using CPI and exchange rates
...
# Apply inflation adjustment: (2023_CPI / year_CPI) * DKK_value / EUR_rate
df[, (monetary_cols) := lapply(.SD, function(x) inf_rate*x/eur_rate), 
   .SDcols = monetary_cols]

# ------------------------------------------------------------------------------
# INCOME PROCESSING AND QUINTILE CREATION
# ------------------------------------------------------------------------------
# Create retirement indicator for income quintile calculation
df[, retired := 0]
df[str_detect(pre_socio, "retired"), retired := 1]

# Create income quintiles based on non-retired population (more stable distribution)
...
# Create year-specific quintiles for temporal comparisons
...
# Remove extreme income outliers (bottom/top 1.5%) to reduce measurement error
...
# ------------------------------------------------------------------------------
# Remove duplicates and observations with missing key income variables
df <- unique(df)
df <- df[!is.na(hh_ind)]   # Require household income
df <- df[!is.na(per_ind)]  # Require personal income

# Fill missing comorbidity scores with zeros
...
# Quality check: ensure all observations have age groups assigned
stopifnot(nrow(df[is.na(age_group)]) == 0)

# Export analysis-ready dataset partitioned by year
df %>%
  group_by(year) %>%
...
# OUTPUT SUMMARY
# ==============================================================================
# Analysis-ready dataset created in data/analysis/01_pre_match/
# - Partitioned by year (2000-2023) 
# - Inflation-adjusted monetary values (2023 EUR)
# - Standardized factor coding and variable names
# - Disease diagnoses merged with socioeconomic panel data
